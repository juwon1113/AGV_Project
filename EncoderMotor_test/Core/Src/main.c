...
/* USER CODE BEGIN PV */
volatile uint8_t control_flag = 0, print_flag = 0;
uint8_t command = 0x00;

uint16_t Base_PWM = 1200;
uint16_t EncoderL_PrevCount, EncoderR_PrevCount;
uint16_t EncoderL_CurrCount, EncoderR_CurrCount;
int16_t EncoderL_DiffCount, EncoderR_DiffCount;
int16_t MotorL_Speed, MotorR_Speed;
...
void SPEED_PID_Control(uint16_t basePwm, float target)
{
	EncoderL_CurrCount = __HAL_TIM_GET_COUNTER(&htim4);
	EncoderR_CurrCount = __HAL_TIM_GET_COUNTER(&htim3);

	EncoderL_DiffCount = ABS((int16_t)(EncoderL_CurrCount - EncoderL_PrevCount));
	EncoderR_DiffCount = ABS((int16_t)(EncoderR_CurrCount - EncoderR_PrevCount));
	...
	PID_Compute(&MOTOR_L, EncoderL_DiffCount);
	PID_Compute(&MOTOR_R, EncoderR_DiffCount);

	MotorL_Speed = basePwm + MOTOR_L.pidOutput;
	MotorR_Speed = basePwm + MOTOR_R.pidOutput;
...
void AGV_ROTATE_LEFT(uint16_t basePwm, float target)
{
...
void AGV_ROTATE_RIGHT(uint16_t basePwm, float target)
{
	HAL_GPIO_WritePin(MOTOR_L_DIR_GPIO_Port, MOTOR_L_DIR_Pin, GPIO_PIN_RESET);
...
void AGV_ROTATE_180(uint16_t basePwm, float target)
{
	HAL_GPIO_WritePin(MOTOR_L_DIR_GPIO_Port, MOTOR_L_DIR_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(MOTOR_R_DIR_GPIO_Port, MOTOR_R_DIR_Pin, GPIO_PIN_SET);
...
int main(void)
{
  ...
  PID_Init(&MOTOR_L, 1.0, 0, 0.0, 0.01, 0, 4320, -4320);
  PID_Init(&MOTOR_R, 1.0, 0, 0.0, 0.01, 0, 4320, -4320);
  ...
  while (1)
  {
	  ...
	  {
	  case CMD_FORWARD:
		  AGV_Forward(1200, 60.0f);
		  break;
	  case CMD_STOP:
		  AGV_Stop();
		  break;
	  case CMD_ROTATE_LEFT:
		  AGV_ROTATE_LEFT(1200, 60.0f);
		  break;
	  case CMD_ROTATE_RIGHT:
		  AGV_ROTATE_RIGHT(1200, 60.0f);
		  break;
	  case CMD_ROTATE_180:
		  AGV_ROTATE_180(1200, 60.0f);
		  break;
	  }
    /* USER CODE END WHILE */
